...
public class WashingMachineTEst {
	public static void main(String[] args) {
		...
		cloth[0]=new Cloth("Cotton","White",1000.0f,"Shirt",true);
		cloth[1]=new Cloth("Nylon","Black",1200.0f,"Pant",true);
		cloth[2]=new Cloth("Polyster","White",700.0f,"Handkerchief",true);
		//System.out.println("wash powder : "+washPowder); //toString is invoked
		
		WashingMachine washingMachine = new WashingMachine();
		WashingPowder powder = new WashingPowder(10, "Surf Excel", "Front Load", true, 40);
		Water water = new Water("Soft", 50, "24");
		Electricity electricity = new Electricity("AC", 240, 4, 100, "Adani");
		
		Laundry laundry = washingMachine.wash(powder, water, electricity, cloth);
		
		float bill;
		bill = laundry.getCostOfWashingPowder() + laundry.getTotalCost() + laundry.getElectricityUsed() + laundry.getWaterUsed();
...
class WashingMachine extends Machine { //isA
	...
	WashingTub washTub = new WashingTub(50,"Metal"); //hasA

	
	
		Laundry wash(WashingPowder washPowder, Water water, Electricity elect, Cloth cloth[]) {
			float totalCost = (float) cloth.length*10;
			float waterCost = (float) water.getQuantity()*0.3f;
			Laundry laundy =new Laundry(cloth.length, 2f,totalCost,waterCost , (elect.getUnitUsed()*elect.getCostPerUnit()), washPowder.getQuantity()*2);
			return laundy;
		}
}
